...
graph TB
    ...
    GW -->|/api/v1/users, /auth,\n/register, /customers, …| US
    GW -->|/api/v1/budgets, /budget-lines,\n/reports, /attachments, …| BS
    GW -->|/api/v1/ai| AI
    GW -->|/api/v1/chat\nSSE streaming| CH
...
- Frontend → Services: via Nginx (single entry point at `:8082`), including a direct route to ai-service (`/api/v1/ai/*`) for account-level settings/model catalog, separate from chat-service's own decision-request path
...
- MCP-shaped tool registry (`list_tools()` / `call_tool()`): the budget toolset is generated by an in-process FastMCP `from_openapi` bridge over budget's OpenAPI spec (`mcp_bridge.py`), with route maps and a dispatch bridge for server-injected resource ids
...
### worker (Celery)
Background jobs shared across services — RabbitMQ as broker, Redis as result backend, per-domain queues (`ai`, `budget`, `users`). Runs a daily scheduled cleanup of expired AI sessions, plus on-demand tasks: verification/invite/password-reset email delivery and feedback notifications.
...
| Tool registry is MCP-shaped from day one | Same consumer-facing interface for hand-curated and OpenAPI→MCP-bridged tools; budget's registry now runs entirely on the OpenAPI→MCP bridge |
