...
async def upload_attachment_view(
    ...
    uploaded = await upload_attachment_service(db, valid_user, report_line_id, file)
    set_span_attributes(attachment_id=uploaded.id)
    return uploaded


@router.get("/by-report-line/{report_line_id}", response_model=List[Attachment])
async def list_attachments_by_report_line_view(
    report_line_id: UUID,
    db: AsyncSession = Depends(get_db),
    valid_user=Depends(get_validated_user),
):
    set_span_attributes(report_line_id=report_line_id)
    return await list_attachments_service(db, valid_user, report_line_id)


@router.get("/{attachment_id}/content")
async def download_attachment_view(
    attachment_id: UUID,
    db: AsyncSession = Depends(get_db),
    valid_user=Depends(get_validated_user),
):
    set_span_attributes(attachment_id=attachment_id)
    attachment, stream = await download_attachment_service(db, valid_user, attachment_id)
...
async def download_attachment_url_view(
    attachment_id: UUID,
    db: AsyncSession = Depends(get_db),
    valid_user=Depends(get_validated_user),
):
    set_span_attributes(attachment_id=attachment_id)
    url = await get_attachment_download_url_service(db, valid_user, attachment_id)
    return RedirectResponse(url, status_code=307)


@router.delete("/{attachment_id}")
async def delete_attachment_view(
    attachment_id: UUID,
    db: AsyncSession = Depends(get_db),
    valid_user=Depends(get_validated_user),
):
    set_span_attributes(attachment_id=attachment_id)
    return {"success": await delete_attachment_service(db, valid_user, attachment_id)}
