...
async def get_budget_lines_by_budget_view(
    ...
    return await get_budget_lines_service(db, budget_id=budget_id, valid_user=valid_user)


@router.get("/{budget_line_id}", response_model=BudgetLine)
async def get_budget_line_by_id_view(
    budget_line_id: UUID,
    db: AsyncSession = Depends(get_db),
    valid_user=Depends(get_validated_user),
):
    set_span_attributes(budget_line_id=budget_line_id)
    return await get_budget_line_by_id_service(
        db,
        valid_user,
        budget_line_id,
    )
...
async def update_budget_line_view(
    budget_line_id: UUID,
    ...
    set_span_attributes(budget_line_id=budget_line_id)
    return await update_budget_line_service(
        db, valid_user=valid_user, budget_line_id=budget_line_id, new_budget_line=budget_line
    )


@router.delete("/{budget_line_id}/")
async def delete_budget_line_view(
    budget_line_id: UUID,
    db: AsyncSession = Depends(get_db),
    valid_user=Depends(get_validated_user),
):
    set_span_attributes(budget_line_id=budget_line_id)
    return await delete_budget_line_service(
        budget_line_id=budget_line_id, valid_user=valid_user, db=db
    )
