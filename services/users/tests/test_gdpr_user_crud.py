...
building an unpersisted UserModel via the factory and passing a mocked
...
class TestSetMarketingConsent:
    async def test_enabling_sets_a_timestamp(self):
        user = UserModelFactory.build(consent_marketing_at=None)
        await set_marketing_consent(AsyncMock(), user, True)
        assert user.consent_marketing_at is not None

    async def test_disabling_clears_the_timestamp(self):
        user = UserModelFactory.build(consent_marketing_at=datetime.now(timezone.utc))
        await set_marketing_consent(AsyncMock(), user, False)
        assert user.consent_marketing_at is None


@pytest.mark.anyio
class TestSoftDeleteUser:
    async def test_scrubs_pii_and_blocks_future_login(self):
        ...
        await soft_delete_user(AsyncMock(), user)
    ...
    async def test_tombstone_email_is_unique_per_user(self):
        user_a = UserModelFactory.build()
        user_b = UserModelFactory.build()
        await soft_delete_user(AsyncMock(), user_a)
        await soft_delete_user(AsyncMock(), user_b)
        assert user_a.email != user_b.email

    async def test_clears_any_pending_email_change(self):
...
class TestEmailChangeRectification:
    def _no_conflict_db(self) -> AsyncMock:
        return self._db_returning(None)

    def _db_returning(self, existing) -> AsyncMock:
        db = AsyncMock()
        result = MagicMock()
    ...
    async def test_new_email_stored_as_pending_old_stays_active(self):
        user = UserModelFactory.build(email="old@example.com", pending_email=None)
        raw_token = await set_pending_email_verification_token(
    ...
    async def test_conflicting_email_rejected(self):
        user = UserModelFactory.build(email="old@example.com")
        other_user = UserModelFactory.build(email="taken@example.com")
        db = self._db_returning(other_user)

        with pytest.raises(ValueError, match="already registered"):
            await set_pending_email_verification_token(db, user, "taken@example.com")

    async def test_confirming_promotes_pending_email(self):
        user = UserModelFactory.build(email="old@example.com")
        await set_pending_email_verification_token(self._no_conflict_db(), user, "new@example.com")

        await mark_email_verified(AsyncMock(), user)
...
class TestEmailChangeRectificationPendingCollision:
    ...
    a mock can't distinguish which column the mocked query "matched
    ...
    async def test_rejected_when_another_user_already_has_it_pending(self, db):
        from app.models.user import UserModel
        ...
        with pytest.raises(ValueError, match="already registered"):
            await set_pending_email_verification_token(db, me, "new@example.com")
