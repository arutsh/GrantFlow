...
class TestStartImpersonation:
    async def test_superuser_can_mint_token_for_any_customer(self):
        ...
        with patch("app.api.auth_routes.get_customer", return_value=customer):
            resp = await start_impersonation(req, _superuser(), db=object())

        assert str(resp.customer_id) == customer.id
        assert resp.customer_name == customer.name

    async def test_non_superuser_is_rejected(self):
        customer = CustomerFactory.build()
        req = ImpersonateRequest(customer_id=customer.id)

        with pytest.raises(HTTPException) as exc:
            await start_impersonation(req, ValidUserFactory(role="admin"), db=object())

        assert exc.value.status_code == 403

    async def test_unknown_customer_returns_404(self):
        req = ImpersonateRequest(customer_id=uuid4())

        with patch("app.api.auth_routes.get_customer", return_value=None):
            with pytest.raises(HTTPException) as exc:
                await start_impersonation(req, _superuser(), db=object())

        assert exc.value.status_code == 404

    async def test_token_is_scoped_to_target_customer_with_admin_equivalent_permissions(self):
        customer = CustomerFactory.build()
        req = ImpersonateRequest(customer_id=customer.id)
    ...
    async def test_token_carries_the_superusers_real_user_id(self):
        customer = CustomerFactory.build()
        req = ImpersonateRequest(customer_id=customer.id)
        superuser = _superuser()

        with patch("app.api.auth_routes.get_customer", return_value=customer):
            resp = await start_impersonation(req, superuser, db=object())

        claims = decode_access_token(resp.access_token)
        assert claims["user_id"] == str(superuser["user_id"])

    async def test_role_flags_reflect_the_target_customer_not_the_superuser(self):
    ...
    async def test_expired_impersonation_token_is_rejected(self):
        customer = CustomerFactory.build()
        req = ImpersonateRequest(customer_id=customer.id)
    ...
    async def test_mint_itself_is_audited(self):
        ...
        with (
            ...
            await start_impersonation(req, superuser, db=object(), request=fake_request)
