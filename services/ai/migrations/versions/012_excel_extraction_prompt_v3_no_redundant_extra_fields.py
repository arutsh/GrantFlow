"""Excel extraction prompt v3: don't duplicate currency data in extra_fields

Revision ID: 012_excel_extract_v3_noextra
Revises: 011_excel_extract_v2_dualcur
...
revision: str = "012_excel_extract_v3_noextra"
down_revision: Union[str, Sequence[str], None] = "011_excel_extract_v2_dualcur"
...
estimated donor-currency equivalent from amount/local_currency/target_currency, so do NOT \
duplicate the donor-currency value (or any other currency conversion) here even when the \
sheet shows one. Only when confidence is low or the row doesn't cleanly fit this shape, include \
the raw cell values you used here (e.g. {"raw_row": ["...", "..."]}) so nothing is silently \
dropped — never as a place to stash a second currency value for an otherwise-resolved line.
...
def upgrade() -> None:
    op.execute(
        sa.text(
            ...
            "WHERE name = 'excel_budget_extraction' AND version = 'v2'"
        )
    )
    op.execute(
        sa.text("""
...
def downgrade() -> None:
    op.execute(
        sa.text("DELETE FROM ai_prompts WHERE name = 'excel_budget_extraction' AND version = 'v3'")
    )
    op.execute(
        sa.text(
