...
#
# Async client (rate_limiter.py's redis.asyncio pattern) — avoids a threadpool hop per request.
...
async def mark_session_revoked(session_id: str, ttl_seconds: int) -> None:
    ...
    if not client:
        return
    try:
        await client.setex(f"{_REVOKED_KEY_PREFIX}{session_id}", ttl_seconds, "1")
    except Exception:
        pass
...
async def is_session_revoked(session_id: str) -> bool:
    ...
    if not client:
        return False
    try:
        return bool(await client.exists(f"{_REVOKED_KEY_PREFIX}{session_id}"))
    except Exception:
        return False
