Workflow rule: **one group = one GitHub ticket = one PR, merged before the next group starts.** Create each ticket with `scripts/new-issue.sh` when that group begins (branch `Frontend/Issue-<n>/<short-desc>`); do not start a group until the previous one's PR is merged. Every PR: `npm run lint` (and any equivalent type-check) clean; commits/pushes only with explicit user approval.

## 1. Foundation — gateway wiring, shared types, API layer
...
## 2. Budget confirmation — ticket depends on 1
...
## 3. Reports list, creation, and read-only detail — ticket depends on 1, 2
...
## 4. Report line CRUD and lifecycle actions — ticket depends on 3
...
## 5. Attachments — ticket depends on 4
...
## 6. Donor dashboard wiring and end-to-end verification — ticket depends on 5
