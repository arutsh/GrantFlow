...
**Grouped/Simple gets its own control, not another `Button variant="toggle"` pill.** The pill is already used for the adjacent Local/Donor/Both currency toggle — a data-*format* control — and reusing it for Grouped/Simple (a data-*structure* control) made the two read as one interchangeable group of options. `components/ui/SegmentedToggle.tsx` is a new, generic two-option joined-track control (sliding thumb, icon + label per option) styled with a teal accent instead of the pill's slate, so the toggle type itself signals what kind of change it makes. `BudgetViewLinesTable` uses it with `Rows3`/`List` icons; it isn't category-specific and is available for a future structural control (e.g. sort mode) without pulling in Button's pill styling.

**Rename is unavailable in Simple view — accepted, not fixed.** The rename pencil is injected via `renderGroupExtra`, which only fires on grouped header rows; Simple mode has none, so the affordance disappears when a user switches to it. This was flagged as confusing during implementation. Decision: leave it — the spec's rename scenario is explicitly scoped to the grouped table, and building a per-row rename in the flat list would mean one pencil per line for the same category, a worse affordance than the current one. The "Simple" toggle button carries a `title` tooltip ("Category rename is only available in Grouped view") as the documentation for this gap.
