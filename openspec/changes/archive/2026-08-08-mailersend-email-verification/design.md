...
**Goals:**
- Send a confirmation email via MailerSend when a user registers, without blocking the registration request.
...
**Non-Goals:**
- General-purpose transactional email templating/marketing email system — this is scoped to the verification flow only; the MailerSend client can be reused later but we're not building a template engine now.
...
**MailerSend product: Email API (HTTPS), not SMTP.**
The worker calls MailerSend's HTTP Email API instead of relaying over SMTP. Rationale: one HTTPS client with an API token is simpler to configure and secret-manage across dev/staging/prod than SMTP host/port/TLS credentials, and it gives us delivery status in the response instead of a fire-and-forget SMTP handoff. MailerSend is also EU-based (Latvia), which better fits the data-residency expectations of a nonprofit-facing product. Alternative considered: SMTP relay — rejected, no material benefit here and more config surface.

**Environment-conditional MailerSend target: trial domain in dev/staging, verified domain in prod.**
Unlike Mailtrap, MailerSend has no separate sandbox API — every account is auto-provisioned with a trial domain that sends through the same Email API but can only deliver to recipient addresses verified on the account (capped at 100 sends total), which is enough for local/dev/CI without risking delivery to real inboxes. Production sends from a verified sending domain instead. Which sender identity is used is driven by env config (e.g. `MAILERSEND_SENDER_DOMAIN`), not a mode flag or a second API surface to branch on.

**Send asynchronously via the existing Celery worker.**
The registration endpoint enqueues a `tasks.users.send_verification_email` task on the already-provisioned `tasks.users` queue and returns immediately. Rationale: registration shouldn't fail or hang on a third-party API call; the queue placeholder already exists and is otherwise unused. Alternative considered: send inline in the request — rejected, couples request latency/availability to MailerSend's.
...
- **[Risk] Celery worker outage silently drops the verification email.** → Mitigation: Celery task-level retry with backoff on the MailerSend call; `resend-verification` endpoint lets the user self-serve after the fact regardless of what happened to the first attempt.
- **[Risk] MailerSend rate limits or transient API failures.** → Mitigation: same retry/backoff as above; failures logged so they show up in the existing Grafana Cloud observability pipeline rather than disappearing silently.
- **[Risk] Trial-domain send cap (100 emails) is exhausted in a shared dev/staging environment.** → Mitigation: verified recipients only in that tier by design; if the cap becomes a real friction point, move staging to a verified low-volume domain rather than raising trial usage.
