...
- **Modified code**: `services/budget/app/models/budget.py` (add `reports` relationship, `actual_currency`, `parent_budget_id`), `app/models/__init__.py`, `app/schemas/__init__.py`, `main.py` (router registration), `services/budget/app/core/config.py` and all three `.env.budget.*` files (new `STORAGE_BACKEND_URL` setting), `services/budget/requirements.txt` (promote `fsspec` from transitive to direct dependency).
- **Database**: new Alembic migration(s) adding `reports`, `report_lines`, `attachments`, `funding_receipts`, `currency_conversions` tables, plus new columns on `budgets` (`actual_currency`, `parent_budget_id`) and `report_lines` (`source_report_id`).
