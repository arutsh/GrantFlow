...
**[Deferred 2026-07-24 — see Open Questions] Donor's own reporting currency is derived via optional `Budget.parent_budget_id`, never stored redundantly.**
...
- `Budget.parent_budget_id` and `resolve_donor_currency()` were dropped from ticket #144 (2026-07-24): audited against tickets #145–148 and confirmed nothing in this change's actual scope ever reads them — they exist solely for the rollup/summary-reporting fast-follow above, which isn't scoped yet either. Re-add both together as part of whichever ticket first needs to display the donor's currency.
